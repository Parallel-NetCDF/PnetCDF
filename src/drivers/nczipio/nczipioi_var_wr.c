...
int nczipioi_save_var (NC_zip *nczipp, NC_zip_var *varp) {
	...
	if (nczipp->rank == varp->chunk_owner[0]) { wcnt += 1; }
	...
	if (varp->zip != NULL) {
		...
		for (l = 0; l < varp->nmychunk; l++) {
			...
			if (varp->dirty[k]) {
				// Apply compression
				err = varp->zip->compress_alloc (varp->chunk_cache[k]->buf, varp->chunksize,
	...
	if (zoffs[varp->nchunk] > 0) {	// No need to do I/O if no dirty chunk to write
		...
		// Record serial
		nczipp->nwrite++;
		err = nczipp->driver->put_att (nczipp->ncp, NC_GLOBAL, "_nwrite", NC_INT, 1,
									   &(nczipp->nwrite), MPI_INT);
		if (err != NC_NOERR) return err;
	...
	for (vid = 0; vid < nvar; vid++) {
		...
		if (varp->zip != NULL) {
			...
			for (l = 0; l < varp->nmychunk; l++) {
				...
				if (varp->dirty[cid]) {
					zdels[ccur] = 1;
					err = varp->zip->compress_alloc (varp->chunk_cache[cid]->buf, varp->chunksize,
		...
		// Sync compressed data size with other processes
		CHK_ERR_IALLREDUCE (zsizesp, zsizes_allp, varp->nchunk, MPI_INT, MPI_MAX, nczipp->comm,
							reqs + vid);
