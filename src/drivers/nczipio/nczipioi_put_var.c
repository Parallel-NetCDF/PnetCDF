...
int nczipioi_put_var_cb_chunk (NC_zip *nczipp,
							   NC_zip_var *varp,
...
int nczipioi_put_var_cb_proc (NC_zip *nczipp,
							  NC_zip_var *varp,
							  const MPI_Offset *start,
							  const MPI_Offset *count,
							  const MPI_Offset *stride,
	...
	do {
		...
		if (wcnt_local[cown] == 0 && cown != nczipp->rank) { smap[cown] = nsend++; }
	...
	nrecv = wcnt_all[nczipp->rank] -
			wcnt_local[nczipp->rank];  // We don't need to receive request from self
	wrange_all[1] *= -1;
	...
	do {
		...
		if (cown != nczipp->rank) {
			...
			*((int *)sbufp[j]) = cid;
			...
			// Data
			packoff = 0;
			CHK_ERR_PACK (buf, 1, ptype, sbufp[j], ssize[j], &packoff, nczipp->comm);
			sbufp[j] += packoff;
			MPI_Type_free (&ptype);
		}
	} while (nczipioi_chunk_itr_next_ex (varp, start, count, citr, &cid, ostart, osize));
	...
	// Read background
	err = nczipioi_load_var_bg (nczipp, varp, nread, rids);
	CHK_ERR
...
int nczipioi_put_var (NC_zip *nczipp,
	...
	switch (nczipp->comm_unit) {
		case NC_ZIP_COMM_CHUNK:
			err = nczipioi_put_var_cb_chunk (nczipp, varp, start, count, stride, buf);
			break;
		case NC_ZIP_COMM_PROC:
			err = nczipioi_put_var_cb_proc (nczipp, varp, start, count, stride, buf);
			break;
	}
	CHK_ERR

	// Write the compressed variable
	err = nczipioi_save_var (nczipp, varp);
	CHK_ERR
