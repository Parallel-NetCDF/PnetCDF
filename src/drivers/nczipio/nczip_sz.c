 ...
 */
int nczip_sz_inq_cpsize(void *in, int in_len, int *out_len, int ndim, int *dims, MPI_Datatype dtype) {
 ...
 */
int nczip_sz_compress(void *in, int in_len, void *out, int *out_len, int ndim, int *dims, MPI_Datatype dtype) {
    int err;
    int i;
    int szdtype;
    size_t r[4];
 ...
 */
int nczip_sz_compress_alloc(void *in, int in_len, void **out, int *out_len, int ndim, int *dims, MPI_Datatype dtype) {
    int err;
    int i;
    int szdtype;
    size_t r[4];
 ...
 */
int nczip_sz_inq_dcsize(void *in, int in_len, int *out_len, int ndim, int *dims, MPI_Datatype dtype) {
    return NC_ENOTSUPPORT;  // sz has no size estimation
}
 ...
 */
int nczip_sz_decompress(void *in, int in_len, void *out, int *out_len, int ndim, int *dims, MPI_Datatype dtype) {
    int err;
    int i;
    size_t r[4];
    int szdtype;
 ...
 * If out_len is not NULL, it will be set to buffer size allocated
 */
int nczip_sz_decompress_alloc(void *in, int in_len, void **out, int *out_len, int ndim, int *dims, MPI_Datatype dtype) {
    int err;
    int i;
    size_t r[4];
    int szdtype;
...
static NCZIP_driver nczip_driver_sz = {
    nczip_sz_init,
    nczip_sz_finalize,
    nczip_sz_inq_cpsize,
    nczip_sz_compress,
    nczip_sz_compress_alloc,
    nczip_sz_inq_dcsize,
    nczip_sz_decompress,
    nczip_sz_decompress_alloc
};

NCZIP_driver* nczip_sz_inq_driver(void) {
    return &nczip_driver_sz;
}
