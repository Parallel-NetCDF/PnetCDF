...
int nczipioi_iget_cb_chunk (NC_zip *nczipp, int nreq, int *reqids, int *stats) {
	...
	for (vid = 0; vid < nczipp->vars.cnt; vid++) {
		if (nreqs[vid] > 0) {
			...
			nczipioi_get_varn_cb_chunk (nczipp, nczipp->vars.data + vid, num, starts, counts, NULL,
										(void **)bufs);
...
int nczipioi_iget_cb_proc (NC_zip *nczipp, int nreq, int *reqids, int *stats) {
	...
	for (i = 0; i < nreq; i++) {
		...
		for (r = 0; r < req->nreq; r++) {
			nczipioi_chunk_itr_init (varp, req->starts[r], req->counts[r], citr,
			...
			do {
				...
				if (rcnt_local[cown] == 0 && cown != nczipp->rank) { smap[cown] = nsend++; }
	...
	nrecv = rcnt_all[nczipp->rank] -
			rcnt_local[nczipp->rank];  // We don't need to receive request form self
	...
	for (i = 0; i < nreq; i++) {
		...
		varp = nczipp->vars.data + req->varid;
		for (r = 0; r < req->nreq; r++) {
			nczipioi_chunk_itr_init_ex (varp, req->starts[r], req->counts[r], citr, &cid, ostart,
...
#endif
	err = nczipioi_load_nvar (nczipp, nread, rids, rlo_all, rhi_all);
	CHK_ERR 
		...
		for (r = 0; r < req->nreq; r++) {
			...
			do {
				if (varp->chunk_owner[cid] == nczipp->rank) {
					...
					err =
						nczipioi_subarray_off_len (varp->ndim, tsize, tssize, tstart, &poff, &plen);
					if (err == 0) {
						plen *= varp->esize;
						poff *= varp->esize;
	...
	for (i = 0; i < nrecv; i++) {
		...
		while (rbufp[j] < rbuf[j] + rsize[j]) {
			...
			err = nczipioi_subarray_off_len (varp->ndim, varp->chunkdim, tssizep, tstartp, &poff,
											 &plen);
			if (err == 0) {
				plen *= varp->esize;
				poff *= varp->esize;
		...
		// Send Response
		CHK_ERR_ISEND (sbuf_re[j], packoff, MPI_BYTE, rstat.MPI_SOURCE, 1, nczipp->comm,
					   sreq_re + j);
#ifdef PNETCDF_PROFILING
