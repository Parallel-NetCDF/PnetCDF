...
int nczipioi_get_varn_cb_chunk (NC_zip *nczipp,
								NC_zip_var *varp,
								int nreq,
...
int nczipioi_get_varn_cb_proc (NC_zip *nczipp,
	...
	// datastructure
	for (req = 0; req < nreq; req++) {
		nczipioi_chunk_itr_init (varp, starts[req], counts[req], citr,
	...
	for (req = 0; req < nreq; req++) {
		...
		do {
			...
			nczipioi_chunk_itr_next_ex (varp, starts[req], counts[req], citr, &cid, ostart, osize));
	}
...
int nczipioi_get_varn (NC_zip *nczipp,
					   NC_zip_var *varp,
					   int nreq,
					   MPI_Offset *const *starts,
					   MPI_Offset *const *counts,
