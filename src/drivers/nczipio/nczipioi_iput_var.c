...
int nczipioi_iput_cb_chunk (NC_zip *nczipp, int nreq, int *reqids, int *stats) {
	...
	for (vid = 0; vid < nczipp->vars.cnt; vid++) {
		if (nreqs[vid] > 0) {
			...
			NC_ZIP_TIMER_PAUSE (NC_ZIP_TIMER_PUT_CB_INIT)

			// Perform collective buffering
			nczipioi_put_varn_cb_chunk (nczipp, nczipp->vars.data + vid, num, starts, counts, NULL,
										(void **)bufs);
...
int nczipioi_iput_cb_proc (NC_zip *nczipp, int nreq, int *reqids, int *stats) {
	...
	nrecv = wcnt_all[nczipp->rank] -
			wcnt_local[nczipp->rank];  // We don't need to receive request form self
