...
int nczipioi_get_var_cb_chunk (NC_zip *nczipp,
							   NC_zip_var *varp,
	...
	do {
		...
		if (varp->chunk_owner[cid] != nczipp->rank) {
			...
			// Send request
			CHK_ERR_ISEND (sbufs[k], packoff, MPI_BYTE, varp->chunk_owner[cid], cid, nczipp->comm,
						   sreqs + k);
			...
			CHK_ERR_IRECV (rbufs[k + nrecv], overlapsize, MPI_BYTE, varp->chunk_owner[cid],
						   cid + 1024, nczipp->comm, rreqs + nrecv + k);
