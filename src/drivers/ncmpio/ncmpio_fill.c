             ...
             void         *buf)
{
    ...
    indx = ncmpio_NC_findattr(&varp->attrs, NC_FillValue);
    if (indx >= 0) {
...
fillerup(NC *ncp)
{
    ...
    for (i=0; i<ncp->vars.ndefined; i++) {
        ...
        indx = ncmpio_NC_findattr(&ncp->vars.value[i]->attrs, NC_FillValue);
...
fill_added(NC *ncp, NC *old_ncp)
{
    ...
    for (; varid<ncp->vars.ndefined; varid++) {
        ...
        /* check if attribute _FillValue is defined */
        indx = ncmpio_NC_findattr(&ncp->vars.value[varid]->attrs, NC_FillValue);
                    ...
                    const void *fill_value) /* when NULL, use default fill value */
{
    ...
    if (fill_value != NULL && !varp->no_fill) {
        ...
        err = ncmpio_put_att(ncdp, varid, NC_FillValue, varp->xtype,
                                                ...
                                                default fill value */
{
    ...
    for (i=0; i<ncap->ndefined; i++) {
        if (strcmp(ncap->value[i]->name, NC_FillValue) == 0)
            break;
    }
