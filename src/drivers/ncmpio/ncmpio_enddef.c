...
NC_begins(NC *ncp)
{
    ...
    if (ncp->old != NULL) {
         ...
         * the new begin_var against the old begin_var.
         */
        if (ncp->begin_var <= ncp->old->begin_var)
            /* do not shrink the header extent */
            ncp->begin_var = ncp->old->begin_var;
        else
            /* file header size has grown out of its old extent, we have to
             * move the data section down. Hence, pick a new one aligned with
             * ncp->v_align.
             */
            ncp->begin_var = D_RNDUP(ncp->begin_var, ncp->v_align);
#endif
               ...
               MPI_Offset  r_align)
{
    ...
    /* check hints from enviornment variable PNETCDF_HINTS, or MPI info */
    read_hints(ncp);
     ...
     * + 2nd priority: hints passed from arguments of ncmpi__enddef()
    ...
    /* determine header extent */
    if (ncp->env_v_align == -1) {
        /* hint nc_var_align_size is not set in PNETCDF_HINTS */
        ncp->v_align = -1;

        if (v_align < 0) {
            /* called from ncmpi_enddef(), v_align is not provided */
            if (ncp->info_v_align >= 0)
                /* use the one set in MPI info when creating/opening file */
                ncp->v_align = ncp->info_v_align;
        }
        else
            /* v_align is passed from ncmpi__enddef */
            ncp->v_align = v_align;

        /* if no fixed-size variables is defined, use r_align */
        if (num_fix_vars == 0 && ncp->v_align == -1) {
            if (ncp->env_r_align == -1) {
                /* hint nc_record_align_size is not set in PNETCDF_HINTS */
                if (r_align < 0) {
                    /* called from ncmpi_enddef(), r_align is not provided */
                    if (ncp->info_v_align >= 0)
                        /* use the one set in MPI info when creating/opening file */
                        ncp->v_align = ncp->info_v_align;
                    else
                        ncp->v_align = NC_DEFAULT_R_ALIGN;
                }
                else
                    ncp->v_align = r_align;
            }
    ...
    else { /* hint nc_var_align_size is set in PNETCDF_HINTS */
        /* use hint, ignore v_align passed from ncmpi__enddef() */
        ncp->v_align = ncp->env_v_align;
    }

    /* determine alignment for record variable section */
    if (ncp->env_r_align == -1) { /* hint nc_record_align_size is not set */
        ncp->r_align = -1;
        ...
        if (r_align < 0) {
            ...
            /* r_align is passed from ncmpi__enddef */
            ncp->r_align = r_align;
        ...
        /* hint nc_record_align_size is set, use it and ignore r_align passed
         * from ncmpi__enddef()
         */
        ncp->r_align = r_align;
