...
/*----< ncmpix_get_uint_$1() >------------------------------------------------*/
static int
ncmpix_get_uint_$1(const void *xp, $1 *ip)
{
    ix_uint xx;
    get_ix_uint(xp, &xx);
    *ip = xx;
...
/*----< ncmpix_put_uint_$1() >------------------------------------------------*/
static int
ncmpix_put_uint_$1(void *xp, const $1 *ip)
{
    ix_uint xx = (ix_uint)(*ip);
    put_ix_uint(xp, &xx);
...
dnl
dnl GETN_UINT(xpp, nelems, tp)
dnl
define(`GETN_UINT',dnl
`dnl
/*----< ncmpix_getn_uint_$1() >-----------------------------------------------*/
int
ncmpix_getn_uint_$1(const void **xpp, MPI_Offset nelems, $1 *tp)
{
    const char *xp = (const char *) *xpp;
    int status = NC_NOERR;

    for ( ; nelems != 0; nelems--, xp += X_SIZEOF_UINT, tp++) {
        const int lstatus = ncmpix_get_uint_$1(xp, tp);
...
dnl
dnl PUTN_UINT(xpp, nelems, tp)
dnl
define(`PUTN_UINT',dnl
`dnl
/*----< ncmpix_putn_uint_$1() >-----------------------------------------------*/
int
ncmpix_putn_uint_$1(void **xpp, MPI_Offset nelems, const $1 *tp)
{
    char *xp = (char *) *xpp;
    int status = NC_NOERR;

    for ( ; nelems != 0; nelems--, xp += X_SIZEOF_UINT, tp++) {
        int lstatus = ncmpix_put_uint_$1(xp, tp);
        if (lstatus != NC_NOERR) status = lstatus;
    }
