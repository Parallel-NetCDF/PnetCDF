        ...
        if (! specp->header_only) {
            ...
            for (varid = 0; varid < nvars; varid++) {
                ...
                if (var.ndims == 0 || var.dims[0] != xdimid ||
                    ...
                    ncmpi_status = ncmpi_inq_att(ncid, varid, NC_FillValue,
                                                 &att.type, &att.len);
                    if (ncmpi_status == NC_NOERR &&
                        att.type == var.type && att.len == 1) {
                        if (var.type == NC_CHAR) {
                            char fillc;
                            NC_CHECK(ncmpi_get_att_text(ncid, varid, NC_FillValue,
                                                        &fillc));
                            var.fillval = fillc;
                        } else
                            NC_CHECK(ncmpi_get_att_double(ncid, varid, NC_FillValue,
                                                          &var.fillval));
                    } else {
                        switch (var.type) {
                            case NC_BYTE:
