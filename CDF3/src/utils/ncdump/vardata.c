...
static void pr_dvals(const struct ncvar *vp, size_t len, const char *fmt,
		     ...
		     const struct fspec* fsp, const int64_t *cor);
static int  upcorner(const size_t* dims, int ndims, int64_t* odom,
		     const size_t* add);
...
annotate(
     ...
     const int64_t *cor,		/* corner coordinates */
     ...
     )
{
	...
	if (fsp->full_data_cmnts) {
	    ...
	    annotate (vp, fsp,  (int64_t*)cor, 0L);
	}
	...
	for (iel = 0; iel < len-1; iel++) {
		...
		Printf(fmt, *vals++);
		Printf(", ");
		annotate (vp, fsp,  (int64_t *)cor, iel);
	    } else {
...
upcorner(
     ...
     int64_t* odom,		/* The "odometer" vector to be updated */
...
vardata(
     const struct ncvar *vp,	/* variable */
     int64_t vdims[],		/* variable dimension sizes */
     int ncid,			/* netcdf id */
     int64_t varid,			/* variable id */
     const struct fspec* fsp	/* formatting specs */
     )
{
    int64_t cor[NC_MAX_DIMS];	/* corner coordinates */
    int64_t edg[NC_MAX_DIMS];	/* edges of hypercube */
    int64_t add[NC_MAX_DIMS];	/* "odometer" increment to next "row"  */
