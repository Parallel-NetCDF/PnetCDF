 ...
 *            float cube(x,y,z);
 *
 *            float time(time);  // coordinate & record variable
 *
 *            float xytime(time, x, y);  // record variable
 *
 *
 *      // global attributes
 *
...
int main(int argc, char **argv) {
  ...
  int status;
  int ncid;
  int64_t dimid1, dimid2, dimid3, udimid;
  int64_t square_dim[2], cube_dim[3], xytime_dim[3], time_dim[1];
  int64_t square_start[2], cube_start[3] = {0, 0, 0};
  int64_t square_count[2] = {50, 50}, cube_count[3] = {100, 50, 50};
  int64_t xytime_start[3] = {0, 0, 0};
  int64_t xytime_count[3] = {100, 50, 50};
  int64_t time_start[1], time_count[1] = {25};
  int64_t square_id, cube_id, xytime_id, time_id;
  static char title[] = "example netCDF dataset";
  static char description[] = "2-D integer array";
  float data[100][50][50], buffer[100];
  int rank;
  int nprocs;
  ...
  xytime_dim[0] = udimid;
  time_dim[0] = udimid;
  status = ncmpi_def_var (ncid, "square", NC_FLOAT, 2, square_dim, &square_id);
  if (status != NC_NOERR) handle_error(status);
  status = ncmpi_def_var (ncid, "cube", NC_FLOAT, 3, cube_dim, &cube_id);
  if (status != NC_NOERR) handle_error(status);
  status = ncmpi_def_var (ncid, "time", NC_FLOAT, 1, time_dim, &time_id);
  if (status != NC_NOERR) handle_error(status);
  status = ncmpi_def_var (ncid, "xytime", NC_FLOAT, 3, xytime_dim, &xytime_id);
  if (status != NC_NOERR) handle_error(status);
  ...
  status = ncmpi_put_vara_float_all(ncid, square_id,
                    square_start, square_count,
                    &data[0][0][0]);
  if (status != NC_NOERR) handle_error(status);
  status = ncmpi_put_vara_float_all(ncid, cube_id,
                    cube_start, cube_count,
                    &data[0][0][0]);
  if (status != NC_NOERR) handle_error(status);
  status = ncmpi_put_vara_float_all(ncid, time_id,
                    time_start, time_count,
                    (void *)buffer);
  if (status != NC_NOERR) handle_error(status);
  status = ncmpi_put_vara_float_all(ncid, xytime_id,
