...
/* process 0 takes the file name as a command-line argument and 
   broadcasts it to other processes */
    ...
    buf = (int **)malloc(ntimes*sizeof(int*));
    if (buf == NULL){
		printf("buf malloc error\n");
                return 0;		
    }	
    varid = (int *)malloc(ntimes*sizeof(int));
    if (varid == NULL){
		printf("varid malloc error\n");
                return 0;		
    }	
    bufcount_list = (MPI_Offset *)malloc(ntimes*sizeof(MPI_Offset));
    if (bufcount_list == NULL){
		printf("bufcount_list malloc error\n");
                return 0;		
    }	
    starts_list = (MPI_Offset **)malloc(ntimes*sizeof(MPI_Offset *));
    if (starts_list== NULL){
		printf("starts_list malloc error\n");
                return 0;		
    }	
    count_list = (MPI_Offset **)malloc(ntimes*sizeof(MPI_Offset *));
    if (count_list == NULL){
		printf("count_list malloc error\n");
                return 0;		
    }	
    datatype_list = (MPI_Datatype*)malloc(ntimes*sizeof(MPI_Datatype));
   ...
   /* subarray in each process is len x len x len */
    ...
    /* mput */
    for (i=0; i<ntimes; i++) {
           starts_list[i][0] = array_of_starts[0]+i*array_of_gsizes[0];
           count_list[i][0]  = length;
        for (j=1; j<ndims; j++) {
...
/*    MPI_Info_set(info, "romio_no_indep_rw", "true");*/
    ...
    for (k=0; k<k_loop; k++){
      sprintf(filename, "%s.%d.%d.%d.%d.nc", pathname, length, ntimes, mvar_flag, k);
      ...
      /* define variables */
        sprintf(varname, "var_%d", 0);
        ncmpi_def_var(ncid, varname, NC_INT, ndims, dimids, &varid[0]);
      for (i=1; i<ntimes; i++){
        varid[i]=varid[0];
      }
...
/* to eliminate paging effects, do the operations once but don't time
   them */
        ...
        if (mvar_flag == 1) {
//      	     status = ncmpi_put_mvara_all(ncid, ntimes, varid,
      	     status = ncmpi_put_mvara_all(ncid, ntimes, varid,
...
/*    ncmpi_get_file_info(ncid, &info); */
    ...
    if (mynod == 0) {
      ...
      fprintf(stderr, "one record variable ntimes:%d, Global array size (%d x %d) x %d x %d integers, local array size: %lld x %lld x%lld\n", ntimes, array_of_gsizes[0], ntimes, array_of_gsizes[1], array_of_gsizes[2],lld(sizes[0]), lld(sizes[1]), lld(sizes[2]));
