...
{
    size_t j;
    int rank, err, nerrs=0, ncid, verbose=0;
    ...
    for (j=0; j<NATTRS; j++) {
        char name[64];
        int ibuf = (int)j;
        sprintf(name, "attr_%zd.x%zd", (j*1747)%8642+100000, (j*8313)%97531+100000);
        err = ncmpi_put_att(ncid, NC_GLOBAL, name, NC_INT, 1, &ibuf);
        CHECK_ERR
    }
...
int main(int argc, char **argv) {
    ...
    err = tst_main(argc, argv, "hashing of large number of gattr", opt, test_io);
