...
program f90tst_vars3
  ...
  ! This program is design to run on 1 process'
  ...
  call check(nf90mpi_create(MPI_COMM_SELF, out_path, cmode, MPI_INFO_NULL, ncid))
  ...
  ! Reopen the file.
  call check(nf90mpi_open(MPI_COMM_SELF, out_path, nf90_nowrite, MPI_INFO_NULL, ncid))
  ...
  if (my_rank .eq. 0) then
      ...
      msg = '*** TESTING F90 '//trim(cmd)//' - def_var API'
