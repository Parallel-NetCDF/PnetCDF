...
int main(int argc, char **argv)
{
    ...
    for(zipdriver = 0; zipdriver < NZIPDRIVER; zipdriver++){
        for(communit = 1; communit < 2; communit++){
            ...
            err = ncmpi_def_dim(ncid, "X", N, dimids);
            CHECK_ERR
            ...
            // Write variable
            start[0] = start[1] = N * rank / np;
            start[1] = 0;
            count[0] = N * (rank + 1) / np - start[0];
            count[1] = N;
            for(i = 0; i < N * N; i++){
                buf[i] = i + 1;
            }
            err = ncmpi_iput_vara_int(ncid, varid, start, count, buf, reqids); CHECK_ERR
            ...
            err = ncmpi_iget_vara_int(ncid, varid, start, count, buf, reqids); CHECK_ERR
