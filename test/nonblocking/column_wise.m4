...
int test_column_wise_$1(const char *out_path, int format, int coll_io, MPI_Info info)
{
    ...
    for (i=0; i<num_reqs; i++) {
        if (reqs[i] != NC_REQ_NULL) { /* add in PnetCDF v1.7.0 */
            fprintf(stderr,"Error at line %d in %s: request ID %d fails to be set to NC_REQ_NULL\n",__LINE__,__FILE__,i);
            nerrs++;
            break;
        }
        if (sts[i] != NC_NOERR) {
    ...
    for (i=0; i<myNX; i++) {
        for (j=0; j<NY; j++) {
            ...
            if (buf[i][j] != expected) {
                fprintf(stderr,"Error at line %d in %s: expect buf[%d][%d]=IFMT($1) but got IFMT($1)\n",
