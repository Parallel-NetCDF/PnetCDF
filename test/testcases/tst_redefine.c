        ...
        MPI_Offset *info_align) /* [3] 0 means unset in MPI info */
{
    ...
    h_minfree = 76;
    v_minfree = 44;
    err = ncmpi__enddef(ncid, h_minfree, 4, v_minfree, 4); CHECK_ERR

    GET_HEADER_SIZE

    /* when ncmpi__enddef called info_align is ignored */
    v_align = (env_v_align) ? env_v_align :
              (!has_fix_vars && env_r_align) ? env_r_align : 4;
    r_align = (env_r_align) ? env_r_align : 4;
    ...
    h_minfree = 0;
    v_minfree = 31;
    err = ncmpi__enddef(ncid, h_minfree, 4, h_minfree, 4); CHECK_ERR
    ...
    /* increase v_align */
    v_align = extent + 4;
    err = ncmpi__enddef(ncid, h_minfree, v_align, v_minfree, 4); CHECK_ERR

    GET_HEADER_SIZE

    /* when ncmpi__enddef called info_align is ignored */
    v_align = (env_v_align) ? env_v_align :
              (!has_fix_vars && env_r_align) ? env_r_align : v_align;
    r_align = (env_r_align) ? env_r_align : 4;
    ...
    /* increase r_align */
    r_align = r_begin + 4;
    err = ncmpi__enddef(ncid, h_minfree, 4, v_minfree, r_align); CHECK_ERR
    ...
    v_align = (env_v_align) ? env_v_align :
              (!has_fix_vars && env_r_align) ? env_r_align : 4;
    r_align = (env_r_align) ? env_r_align : r_align;
