...
check_vars(MPI_Comm    comm,
           ...
           MPI_Offset *count)
{
    int i, j, nerrs=0, err, rank, *buf, rec_dim, dimids[3], nvars;
    ...
    for (i=0; i<nvars; i++) {
        err = ncmpi_inq_vardimid(ncid, i, dimids); CHECK_ERROUT
        if (dimids[0] == rec_dim) {
            err = ncmpi_get_vara_int_all(ncid, i, start, count, buf);
            CHECK_ERR
        }
        else {
            err = ncmpi_get_vara_int_all(ncid, i, start+1, count+1, buf);
            CHECK_ERR
        }
        for (j=0; j<count[1] * count[2]; j++) {
            int expect = rank + j + i * 10;
            if (buf[j] != expect) {
                char name[64];
                err = ncmpi_inq_varname(ncid, i, name);
                printf("Error: variable %s [%d] expect %d but got %d\n",
                       name, j, expect, buf[j]);
                nerrs++;
                goto err_out;
            }
