  ...
  for (i = 0; i < ngatts; i++) {
	...
	free(valuep);
      ...
      case NC_SHORT:
        ...
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_att_short (ncid2, NC_GLOBAL, name, type, len, (short *)valuep);
        if (status != NC_NOERR) handle_error(status);
        free(valuep);
        break;
      case NC_INT:
        ...
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_att_int (ncid2, NC_GLOBAL, name, type, len, (int *)valuep);
        if (status != NC_NOERR) handle_error(status);
        free(valuep);
        break;
      case NC_FLOAT:
        ...
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_att_float (ncid2, NC_GLOBAL, name, type, len, (float *)valuep);
        if (status != NC_NOERR) handle_error(status);
        free(valuep);
        break;
      case NC_DOUBLE:
        ...
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_att_double (ncid2, NC_GLOBAL, name, type, len, (double *)valuep);
        if (status != NC_NOERR) handle_error(status);
        free(valuep);
        break;
      default:
    ...
    for (j = 0; j < varnatts[i]; j++) {
	  ...
	  free(valuep);
        ...
        case NC_SHORT:
          ...
          if (status != NC_NOERR) handle_error(status);
          status = ncmpi_put_att_short (ncid2, varids[i], name, type, len, (short *)valuep);
          if (status != NC_NOERR) handle_error(status);
          free(valuep);
          break;
        case NC_INT:
          ...
          if (status != NC_NOERR) handle_error(status);
          status = ncmpi_put_att_int (ncid2, varids[i], name, type, len, (int *)valuep);
          if (status != NC_NOERR) handle_error(status);
          free(valuep);
          break;
        case NC_FLOAT:
          ...
          if (status != NC_NOERR) handle_error(status);
          status = ncmpi_put_att_float (ncid2, varids[i], name, type, len, (float *)valuep);
          if (status != NC_NOERR) handle_error(status);
          free(valuep);
          break;
        case NC_DOUBLE:
          ...
          if (status != NC_NOERR) handle_error(status);
          status = ncmpi_put_att_double (ncid2, varids[i], name, type, len, (double *)valuep);
          if (status != NC_NOERR) handle_error(status);
          free(valuep);
          break;
	default:
	  /* XXX: handle unexpected types */
      }
    }
  }
  ...
  for (i = 0; i < nvars; i++) {
    ...
    if (isRecvar) switch (vartypes[i]) {
      ...
      case NC_SHORT:
        ...
        status = ncmpi_get_vars_short_all(ncid1, i, start, shape, stride, (short *)valuep);
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_vars_short_all(ncid2, varids[i],
	...
	valuep = (void *)malloc(varsize * sizeof(int));
        status = ncmpi_get_vars_int_all(ncid1, i, start, shape, stride, (int *)valuep);
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_vars_int_all(ncid2, varids[i],
	...
	break;
      case NC_FLOAT:
        ...
        status = ncmpi_get_vars_float_all(ncid1, i, start, shape, stride, (float *)valuep);
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_vars_float_all(ncid2, varids[i],
      ...
      case NC_DOUBLE:
        ...
        status = ncmpi_get_vars_double_all(ncid1, i, start, shape, stride, (double *)valuep);
        if (status != NC_NOERR) handle_error(status);
        status = ncmpi_put_vars_double_all(ncid2, varids[i],
