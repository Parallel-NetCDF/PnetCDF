...
/*
 * Test ncmpi_create
 *    For mode in NC_NOCLOBBER, NC_CLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL do:
...
{
    int clobber;		/* 0 for NC_NOCLOBBER, 1 for NC_CLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL */
    int err;
    int ncid;
	...
	err = ncmpi_create(comm, scratch, clobber ? NC_CLOBBER : NC_NOCLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL, &ncid);
	IF (err)
    ...
    err = ncmpi_create(comm, scratch, NC_NOCLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL, &ncid);
    IF (err != NC_EEXIST)
...
test_ncmpi_redef(void)
{
    ...
    err = ncmpi_create(comm, scratch, NC_NOCLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL, &ncid);
    /* err = ncmpi__create(scratch, NC_NOCLOBBER|NC_64BIT_OFFSET, 0, &sizehint, &ncid); */
    IF (err) {
        error("ncmpi_create: %s", ncmpi_strerror(err));
        return;
    }
...
test_ncmpi_sync(void)
{
    ...
    err = ncmpi_create(comm, scratch, NC_NOCLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL, &ncidw);
    IF (err) {
        error("ncmpi_create: %s", ncmpi_strerror(err));
	return;
    }
...
test_ncmpi_abort(void)
{
    ...
    err = ncmpi_create(comm, scratch, NC_CLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL, &ncid);
    IF (err) {
        error("ncmpi_create: %s", ncmpi_strerror(err));
        return;
    }
...
test_ncmpi_copy_att(void)
{
    ...
    IF (err)
        error("ncmpi_open: %s", ncmpi_strerror(err));
    err = ncmpi_create(comm, scratch, NC_NOCLOBBER|NC_64BIT_OFFSET, MPI_INFO_NULL, &ncid_out);
    IF (err) {
        error("ncmpi_create: %s", ncmpi_strerror(err));
        return;
    }
