...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * This example makes a number of nonblocking API calls, each to write a single
 * columns of a 2D integer array. Each process writes NX columns and any two
 * consecutive columns are of nprocs columns distance apart from each other.
 * In this case, the fileview of each column overlaps with all other columns.
 ...
 * flatten file offsets must be monotonically non-decreasing. PnetCDF handles
 ...
 * them in an increasing order. The sorted pairs are used to construct a
 * fileview that meets the monotonically non-decreasing offset requirement,
 * and hence the nonblocking requests can be serviced by a single MPI-IO call. 
 * 
